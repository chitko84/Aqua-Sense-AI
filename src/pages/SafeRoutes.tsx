                 ...
                 >
                    ...
                    <div className="bg-slate-950 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group">
                       ...
                       <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                          <div className="w-24 h-24 bg-cyan-600/10 rounded-[2.5rem] flex items-center justify-center border border-cyan-500/20 shadow-inner group-hover:rotate-6 transition-transform">
                             <CheckCircle2 className="w-12 h-12 text-cyan-400" />
                          </div>
                          <div className="flex-1 space-y-4 text-center md:text-left">
                             <div>
                                <h3 className="text-xs font-black text-cyan-400 uppercase tracking-[0.4em] mb-2 flex items-center justify-center md:justify-start gap-2">
                                  <div className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-pulse"></div> Sentinel Recommends
                                </h3>
                                <h4 className="text-4xl font-black tracking-tighter uppercase whitespace-nowrap">Safest Extraction Corridor</h4>
                             </div>
                          ...
                          <button className="px-10 py-5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-xl shadow-cyan-900/40 transition-all active:scale-95 flex items-center gap-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       {[results.fastest, results.safest].map((route) => (
                          <div 
                            key={route.id}
                            className={`bg-white rounded-[2.5rem] border overflow-hidden shadow-sm hover:shadow-xl transition-all flex flex-col ${
                              route.isSafest ? 'border-cyan-500 ring-4 ring-cyan-500/5' : 'border-slate-200'
                            }`}
                          >
                             <div className={`p-8 border-b ${route.isSafest ? 'bg-cyan-50/50 border-cyan-100' : 'bg-slate-50 border-slate-100'} flex items-center justify-between`}>
                                <div className="flex items-center gap-3">
                                   <div className={`p-2.5 rounded-xl ${route.isSafest ? 'bg-cyan-600 text-white shadow-lg' : 'bg-slate-900 text-white shadow-lg'}`}>
                                      {route.isSafest ? <ShieldAlert className="w-5 h-5" /> : <Clock className="w-5 h-5" />}
                                   </div>
                             ...
                             <div className="p-8 space-y-8 flex-1">
                                <div className="flex justify-between items-end">
                                   <div className="space-y-1">
                                      ...
                                      <p className="text-4xl font-black text-slate-900 tracking-tighter">{route.duration}</p>
                                   </div>
                                   <div className="text-right space-y-1">
                                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Distance</p>
                                      <p className="text-xl font-black text-slate-900 tracking-tight">{route.distance}</p>
                                   </div>
                                </div>
                                ...
                                <div className="space-y-4 pt-6 border-t border-slate-50">
                                   ...
                                   <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                      {[
                                        ...
                                        <div key={i} className="bg-slate-50 p-3 rounded-xl flex flex-col justify-between border border-slate-100">
                                           <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{f.label}</p>
                                           <div className="flex items-baseline gap-1">
                                              <p className="text-[11px] font-black text-slate-900 uppercase">{f.val}</p>
                                              <p className="text-[5px] font-black text-slate-400 uppercase">{f.sub}</p>
                                           </div>
                                        </div>
                                      ))}
                                   </div>
                                ...
                                <div className={`p-5 rounded-2xl border italic text-[11px] font-black ${
