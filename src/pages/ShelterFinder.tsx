    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase">Shelter Network</h1>
          <p className="text-slate-500 text-sm font-medium italic">Live synchronization of <span className="text-cyan-600 font-bold">Sanctuary Nodes</span> and Ground Logistics.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
           ...
           <div className="relative group">
              ...
              <input 
                ...
                className="pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-[11px] font-black uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-cyan-500/10 transition-all w-48"
                value={searchQuery}
      ...
      {recommendedShelter && (
        ...
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
             <div className="w-20 h-20 bg-cyan-500/10 rounded-3xl flex items-center justify-center border border-cyan-500/20 shadow-inner group-hover:rotate-6 transition-transform">
                <Zap className="w-10 h-10 text-cyan-400" />
             </div>
             <div className="flex-1 space-y-4">
                <div>
                   <h3 className="text-xs font-black text-cyan-400 uppercase tracking-[0.3em] mb-2 flex items-center gap-2">
                     <div className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-pulse"></div> Smart Sanctuary Match
                   </h3>
                   <h4 className="text-2xl font-black tracking-tight uppercase">{recommendedShelter.name}</h4>
                </div>
                <div className="flex flex-wrap gap-x-6 gap-y-3">
                   <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      <MapPin className="w-3.5 h-3.5 text-cyan-600" /> {recommendedShelter.distance} From Current GPS
                   </div>
                   <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      <Users className="w-3.5 h-3.5 text-emerald-500" /> {recommendedShelter.availableSpace} Vacant Nodes
                   </div>
                   <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      <Navigation className="w-3.5 h-3.5 text-blue-500" /> Low Route Risk
                   </div>
                </div>
                <div className="bg-white/5 border border-white/10 p-4 rounded-xl">
                   <p className="text-[11px] text-slate-400 font-medium italic leading-relaxed">
                     This shelter is recommended because it has {recommendedShelter.availableSpace} available spaces, low route risk, and essential facilities such as {recommendedShelter.facilities.join(', ').toLowerCase()}.
                   </p>
                </div>
             </div>
